...
An end-to-end data analysis project using a synthetic cafe sales dataset from Kaggle. The dataset was intentionally dirty to simulate a realistic data cleaning scenario. The project covers the full pipeline — from raw dirty data, through SQL cleaning and EDA, to a business-ready Power BI dashboard.
...
- **POS System Issue:** Unknown/blank/error location entries outnumber both In-store (2,731) and Takeaway (2,711) valid records — strong indicator of a bug or recording failure in the cafe's Point of Sale system
...
- **Recomputed missing `Total Spent`** using `Quantity x Price Per Unit` for rows where value was `'ERROR'`, `'UNKNOWN'`, or blank
...
- **Converted column types:** `Total Spent` to `DOUBLE`, `Transaction Date` to `DATE`
...
- **Location analysis:** Counted transactions and summed revenue by Location — flagged that unknown/error entries exceed both In-store and Takeaway counts, pointing to a POS recording issue; In-store and Takeaway are closely matched, showing the cafe serves both equally
...
| `EDA_cafe.sql` | SQL query script for exploratory analysis questions and rolling totals |
| `cafe_sales_cleaned.csv` | Cleaned dataset exported after SQL cleaning |
| `CAFE DASHBOARD.pbix` | Power BI file containing data models, canvas reports, and visuals |
| `Balagtas, Niel_Cafe Sales Documentation.pdf` | Full project documentation covering methodology and findings |
| `cafe-dashboard-screenshot.png` | Dashboard screenshot for preview |

---

## Dashboard Preview

![Cafe Sales Dashboard](cafe-dashboard-screenshot.png)

---

## Conclusions

The cafe is in a healthy and stable state based on 2023 data — consistent monthly revenue with no losing months, a steadily climbing rolling total, and a well-balanced payment method distribution. The $8.94 average spend per order signals strong multi-item purchasing behavior, presenting an opportunity to push the average toward $10 through promotions or combo deals.

Although Coffee dominates in units sold, Salad is the real revenue driver at $5 per unit — the cafe could leverage this by promoting salad pairings or combo meals.
